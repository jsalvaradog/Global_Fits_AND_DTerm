...
class Theory(object):
    ...
    """
    def __init__(self, nproc=4, **kwargs) -> None:
    ...
    def _chi2_point(self, args):
        pt, asym, kwargs = args

        diff = self.predict(pt, observable=pt.observable, **kwargs) - pt.val

        if asym:
            err = pt.errplus if diff > 0 else pt.errminus
        else:
            err = pt.err
    ...
    def chisq_single(self, points, asym=False, **kwargs):
        tasks = [(pt, asym, kwargs) for pt in points]
        return sum(self.pool.map(self._chi2_point, tasks))
