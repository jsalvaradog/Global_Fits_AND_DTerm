...
class MellinBarnes(object):
    ...
    def _mellin_barnes_integral(self, xip, wce, gpd):
        """Return convolution of evolved Wilson coefs and GPDs."""
        #xip of shengying, same xi of Muller
        eph = np.exp(self.phi*1j)
        cfacj = eph * np.exp((self.jpoints + 1) * log(1/abs(xip))) # eph/xip**(j+1)
    ...
    def _mellin_barnes_integral_HE(self, xip, wce, H, E):
        ...
        eph = np.exp(self.phi*1j)        
        cfacj = eph * np.exp((self.jpoints + 1) * log(1/xip)) # eph/xip**(j+1)
        cch_H = np.einsum('j,sa,sja,ja->j', cfacj,
                          self.pw_strengths(), wce, H)
        cch_E = np.einsum('j,sa,sja,ja->j', cfacj,
                          self.pw_strengths_E(), wce, E)
        imh = sign*np.dot(self.wg, cch_H.imag) #Provisional solution
        ime = sign*np.dot(self.wg, cch_E.imag) #The Mellin-Barnes contribution gives several sign changes for DDVCS
        np.multiply(cch_H, self.tgj, out=cch_H) #Version of the model in the farm is different and outdated.
