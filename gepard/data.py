...
class DataSet(list):
    ...
    def df(self):
        ...
        attrs = ['observable', 'collaboration', 'id', 'x', 'eta',
                 'xi', 'xip', 'xB', 'Q2', 'Qp2', 't', 'tm', 'in1energy', 'W',
                 'phi', 'FTn', 'varFTn', 'val',
...
def _fill_kinematics(kin, old={}):
    ...
    Complete set of kinematical variables is {xB, t, Q2,, W, s, xi, xip, tm, phi}.
    ...
    if 'xB' in kin:
        if 'Qp2' in kin and kin.Qp2 != 0:
            kin.xi = (kin.Q2 + kin.Qp2) / ( (2.*kin.Q2/kin.xB) - kin.Q2 - kin.Qp2 + kin.t )
            kin.xip = (kin.Q2 - kin.Qp2) / ( (2.*kin.Q2/kin.xB) - kin.Q2 - kin.Qp2 + kin.t )
        else:
            kin.xi = kin.xB / (2. - kin.xB)
            kin.xip = kin.xB / (2. - kin.xB)
    duo = set(['t', 'tm'])
